# R script to merge GSE182407 data and perform DESeq2 analysis (fixed version)
...
merge_sample_files <- function(data_dir) {
    ...
    for (i in 1:length(csv_files)) {
        ...
        expr_matrix[, i] <- as.numeric(sample_data$Frag_count)
...
create_phenotype_data <- function(expr_matrix) {
    ...
    # Sample names like: GSM5528434_LN_C1, GSM5528435_LN_C2, etc.
    
    pheno_data <- data.frame(
        sample_id = sample_names,
        cell_line = sapply(strsplit(sample_names, "_"), function(x) {
            if (length(x) >= 3) x[2] else "Unknown"
        }),
