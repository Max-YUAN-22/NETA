...
# 处理补充数据文件 - 修复版本
process_supplementary_files_fixed <- function() {
  cat("=== 修复GSE182407数据处理 ===\n")
  ...
  for (file in data_files) {
    ...
    tryCatch({
      ...
      if ("Gene_symbol" %in% colnames(data) && "Frag_count" %in% colnames(data)) {
        # 过滤掉空基因名
        valid_rows <- !is.na(data$Gene_symbol) & data$Gene_symbol != ""
        
        gene_counts <- data$Frag_count[valid_rows]
  ...
  # 保存合并后的数据
  write.csv(expr_matrix, "data/raw/GSE182407/GSE182407_expression_matrix_fixed.csv", row.names = TRUE)
...
# 创建表型数据
create_phenotype_data <- function() {
  cat("\n=== 创建表型数据 ===\n")
  ...
  # 保存表型数据
  write.csv(pheno_data, "data/raw/GSE182407/GSE182407_phenotype_data_fixed.csv", row.names = TRUE)
...
# 主函数
main <- function() {
  cat("=== GSE182407数据处理修复开始 ===\n")
  
  # 处理数据
  expr_matrix <- process_supplementary_files_fixed()
  
  if (!is.null(expr_matrix)) {
    # 创建表型数据
    pheno_data <- create_phenotype_data()
    
    cat("\n✅ GSE182407数据处理修复完成！\n")
    cat("表达矩阵维度:", dim(expr_matrix), "\n")
    cat("表型数据维度:", dim(pheno_data), "\n")
    cat("数据已保存到:\n")
    cat("  - data/raw/GSE182407/GSE182407_expression_matrix_fixed.csv\n")
    cat("  - data/raw/GSE182407/GSE182407_phenotype_data_fixed.csv\n")
  } else {
    cat("\n❌ GSE182407数据处理修复失败\n")
  }
}
