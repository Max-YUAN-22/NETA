...
# 检查哪些数据集有RNA-seq数据
check_rnaseq_datasets <- function() {
  datasets <- c(
  ...
  for (geo_id in datasets) {
    ...
    if (file.exists(expr_file)) {
      # 检查文件大小（RNA-seq数据通常很大）
      file_size <- file.info(expr_file)$size
      if (file_size > 1000000) {  # 大于1MB
        # 读取前几行检查数据
        expr_data <- read.csv(expr_file, row.names = 1, check.names = FALSE, nrows = 5)
        if (nrow(expr_data) > 0 && ncol(expr_data) > 0) {
          rnaseq_datasets <- c(rnaseq_datasets, geo_id)
          cat("✅", geo_id, "- RNA-seq数据 (", round(file_size/1024/1024, 1), "MB)\n")
        }
      } else {
        cat("❌", geo_id, "- 微阵列数据 (", round(file_size/1024, 1), "KB)\n")
      }
    }
  }
  
  return(rnaseq_datasets)
}
...
run_deseq2_analysis <- function(geo_id) {
  ...
  tryCatch({
    ...
    for (col in colnames(pheno_data)) {
      if (grepl("group|condition|treatment|disease|type|source_name", col, ignore.case = TRUE)) {
        group_col <- col
        break
      }
    }
    ...
    if (is.null(group_col)) {
      ...
      # 清理分组名称
      groups <- gsub(".*: ", "", groups)  # 移除前缀
      groups <- gsub("\\s+", "_", groups)  # 替换空格
    }
...
# 主程序
cat("=== 识别RNA-seq数据集 ===\n")
rnaseq_datasets <- check_rnaseq_datasets()

cat("\n=== 开始DESeq2分析 ===\n")
cat("RNA-seq数据集:", paste(rnaseq_datasets, collapse = ", "), "\n")
cat("总数据集数:", length(rnaseq_datasets), "\n\n")

success_count <- 0
failed_datasets <- c()

for (geo_id in rnaseq_datasets) {
