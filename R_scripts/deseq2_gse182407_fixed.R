# Fixed DESeq2 analysis for GSE182407
...
# Convert comparison_group to factor
...
perform_deseq2_analysis <- function(expr_matrix, pheno_data, analysis_name) {
    ...
    # Filter significant genes (relaxed criteria for demonstration)
    significant_genes <- subset(res, padj < 0.1 & abs(log2FoldChange) > 0.5)
    cat("显著差异基因数 (padj < 0.1, |log2FC| > 0.5):", nrow(significant_genes), "\n")
